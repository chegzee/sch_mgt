...
class GoogleDriveUploadService
{
    ...
    public function getAccessToken($clientId, $authorizedRedirectURI, $clientSecret)
    {
        // echo json_encode(["status"=> false, "message"=> $_SESSION['code']]);exit;

        $curlPost = 'client_id=' . $clientId . '&redirect_uri=' . $authorizedRedirectURI . '&client_secret=' . $clientSecret . '&code=' . $_SESSION['code'] . '&grant_type=authorization_code';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, GOOGLE_OAUTH2_TOKEN_URI);
    ...
    public function uploadFileToGoogleDrive($accessToken, $fileContent, $filetype, $fileSize)
    {
        ...
        curl_setopt($curl, CURLOPT_URL, GOOGLE_DRIVE_FILE_UPLOAD_URI . '?uploadType=media');
    ...
    {
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, GOOGLE_DRIVE_FILE_META_URI . $googleDriveFileId);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_POST, 1);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false);
